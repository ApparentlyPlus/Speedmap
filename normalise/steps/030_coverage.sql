-- One row per filed service. The register files (coverid, servprov, technolo) more than
-- once, so distinct on picks the most recent filing rather than letting on conflict
